...
bool DirectCache::requestMemoryAddress(unsigned int address) {
    ...
    bool hit = false;
	 ...
	 // uses the formulas to get the tag, cache line and cache word
  ...
  int tag = ((address & tag_mask) >> (int) (log2 (number_of_words_in_block) + log2(number_of_lines_in_cache)));
	...
	// increments the number of memory requests after doing the formulas
	number_of_memory_requests++;

	// if program finds a hit...
    ...
    unsigned int hitPercent; 
    
	  //uses the formula to calculate hit percent
  
    hitPercent = (number_of_hits  * 100 / number_of_memory_requests);
...
int main(int argc, char* argv[])
{
    ...
    if ((number_of_bits_defining_block_size > 0) && (number_of_bits_defining_block_size < 6))
    {
